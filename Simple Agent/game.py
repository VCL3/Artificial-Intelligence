...
class Agent:
    ...
    def getAction(self, state):
        ...
        sonar}.py) and must return an action from Directions.{North,
...
class Configuration:
    """A Configuration holds the (x,y) coordinate of a character, along
...
class Grid:
    """A 2-dimensional array of objects backed by a list of lists.  Data
    is accessed via grid[x][y] where (x,y) are positions on a Pacman
    map with x horizontal, y vertical and the origin (0,0) in the
    bottom left corner.

    The __str__ method constructs an output that is oriented like a
    pacman board.
